...
export default function NordDashboard() {
  return <main className={styles.page}>
    <header><Link href="/" className={styles.brand}><i>C</i><span>Clovo</span></Link><nav><a className={styles.active}>Oversikt</a><a>Portefølje</a><a>Relasjoner</a><a>Rapporter</a></nav><div><span>EKSEMPELDATA</span><i>IH</i></div></header>
    ...
    <section className={styles.storyGrid}>
      ...
      <article className={styles.relationships}><div className={styles.sectionTop}><span>RELASJONSHELSE</span><MoreHorizontal /></div><div className={styles.ring}><div><strong>84</strong><span>av 100</span></div></div><p>Porteføljen har stabil kontaktfrekvens. To kontoer mangler planlagt oppfølging.</p><div className={styles.legend}><span><i className={styles.good}/>Sterk 12</span><span><i className={styles.watch}/>Følg med 4</span><span><i className={styles.risk}/>Risiko 2</span></div></article>

      <article className={styles.forecast}><div className={styles.sectionTop}><span>PROGNOSE · 6 MÅNEDER</span><button>Netto verdi</button></div><div className={styles.bars}>{[42,58,49,76,68,91].map((h,i)=><div key={i}><i style={{height:`${h}%`}} className={i===5?styles.current:""}/><span>{['Mar','Apr','Mai','Jun','Jul','Aug'][i]}</span></div>)}</div><div className={styles.forecastFooter}><p><span>Bekreftet</span><strong>3,14m</strong></p><p><span>Vektet</span><strong>1,68m</strong></p><p><span>Totalt</span><strong>4,82m</strong></p></div></article>

      <article className={styles.agenda}><div className={styles.sectionTop}><span>NESTE 48 TIMER</span><CalendarDays /></div>{[['09:00','Intern status','Team Nord'],['10:30','Behovsmøte','Fjell & Fjord'],['13:00','Tilbudsgjennomgang','Norsk Form'],['15:15','Oppfølging','Edda Gruppen']].map((x,i)=><div className={styles.event} key={x[0]}><time>{x[0]}</time><span><b>{x[1]}</b><small>{x[2]}</small></span><i className={i===1?styles.eventActive:""}><Circle /></i></div>)}<button className={styles.add}><Plus /> Legg til aktivitet</button></article>
    </section>
